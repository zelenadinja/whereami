...
def aug_version_1(config: dict, train: bool) -> Callable:
    ...
    if train:
        return albumentations.Compose(
            [
                albumentations.SmallestMaxSize(config['train_small_max_size']),
                albumentations.RandomCrop(config['train_crop'], config['train_crop']),
    ...
    return albumentations.Compose(
        [
            ...
            albumentations.CenterCrop(config['valid_crop'], config['valid_crop']),
            albumentations.Normalize(config['mean'], config['std']),
            ToTensorV2(),
        ]
    )