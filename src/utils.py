...
def artifact_to_s3(object_: Any, bucket: str, key: str, extension: str = 'json', verbose: bool = True) -> bool:
    ...
    if extension == 'json':
        streaming_object = io.BytesIO(json.dumps(object_).encode()) #Make json object, encode it since BytesIO expects bytes
    elif extension == 'yaml':
        streaming_object = io.BytesIO(yaml.safe_dump(object_).encode()) #Same with yaml
    elif extension == 'pkl':
        streaming_object = io.BytesIO(pickle.dumps(object_)) #Mostly for objects like label encoder
...
def _upload_file_obj(file_like_object: io.BytesIO, key: str, bucket: str, extension: str, verbose: bool) -> bool:
    ...
    try:
        if verbose:
            with tqdm.tqdm(total=filesize, unit='B', unit_scale=True, ascii=True, desc=f'Uplaoding {key} to S3 Bucket') as pbar:
                s3client.upload_fileobj(Fileobj=file_like_object,
                                        Bucket=bucket,
                                        Key=key + '.' + extension,
