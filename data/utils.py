...
def get_landmark_ids(dataframe: pd.DataFrame, num_images: int) -> Dict[str, int]: 
    ...
    num_images: int
        Minimum number of images which class contains, for example if num_images=200, it will return ids that have atleast 200 images.
...
def get_image_fpaths(dataframe:pd.DataFrame, train:bool) -> pd.Series:
    ...
    image_fpaths = dataframe['id'].map(lambda image_id: os.path.join(SUB_DIR, image_id[0], image_id[1], image_id[2], image_id + '.jpg'))
...
def label_encoder(dataframe:pd.DataFrame, target_column: str = 'landmark_id') -> Dict[str, Union[sklearn.preprocessing.LabelEncoder, np.ndarray]]:
    ...
    """
    la = LabelEncoder()
    encoded_target = la.fit_transform(dataframe[target_column])
